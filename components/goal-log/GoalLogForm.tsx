...
type GoalLogFormProps = {
  ...
  // false の場合、対象月が既に始まっているため目標本文（技術・フィジカル・アクションプラン）は
  // 修正できない（Daily Logと同様、コーチが見た後の後出し修正を防ぐ方針）。
  // 進捗（ProgressScale）は月の途中で変化していくのが前提の項目のため、この制限の対象外。
  editable?: boolean;
};

export function GoalLogForm({ defaultMonthValue, initial, editable = true }: GoalLogFormProps) {
  ...
  const canSubmit = parsed.success && !isPending && editable;
    ...
    <div className="mx-auto flex max-w-md flex-col gap-8 pb-44">
      {!editable && (
        <p className="rounded-lg bg-slate-100 p-3 text-sm font-medium text-slate-600">
          この月はすでに始まっているため、目標の内容（強化テーマ・フィジカル目標・アクションプラン）は修正できません。進捗の記録は引き続き更新できます。
        </p>
      )}

      <fieldset disabled={!editable} className="flex flex-col gap-8 disabled:opacity-60">
        <div>
